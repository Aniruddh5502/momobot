...
def write(filePath: str, content: str) -> dict:
    ...
    try:
        ...
        try:
            ...
            console.print(f"{theme_char} [dim red]Write failed: {filePath} gets out of workspace[/dim red]")
        ...
        console.print(f"{theme_char} [dim]{filePath} written succesfully[/dim]")
        return create_tool_response(
            status="success",
...
def edit(path: str, old_str: str, new_str: str) -> str:
    ...
    try:
        full_path.relative_to(workspace_resolved)
                ...
                console.print(f"{theme_char} [dim red]Edit failed: Path outside workspace[/dim red]"); return create_tool_response(
            status="error",
            error_message=f"Error: Path {path} is outside the workspace boundary."
        )

    if not full_path.exists():
        if not full_path.is_file():
                    console.print(f"{theme_char} [dim red]Edit failed: Not a file[/dim red]"); return create_tool_response(
                status="error",
    ...
    if count == 0:
                console.print(f"{theme_char} [dim red]Edit failed: String not found[/dim red]"); return create_tool_response(
            status="error",
            error_code="OLD_STR_NOT_FOUND",
            error_message=f"Error: 'old_str' not found in {path}.\n Tip: Make sure whitespace and indentation match exactly."
        )
    if count > 1:
                console.print(f"{theme_char} [dim red]Edit failed: String not unique[/dim red]"); return create_tool_response(
            status="error",
            error_code="MULTIPLE_INSTANCE_OF_SAME_STRING",
            error_message=f"Error: 'old_str' appears {count} times in {path}. It must be unique. Add more context to make it unambiguous."
        )
    ...
    if not diff_lines:
                console.print(f"{theme_char} [dim red]Edit failed: No changes made[/dim red]"); return create_tool_response(
            status="error",
...
def listDir(dirPath: str="") -> dict:
    ...
    try:
        ...
        for item in items:
            ...
            (f"[bold dim]{_NEST} {item.name}{'/' if item.is_dir() else ''}[bold dim]")
